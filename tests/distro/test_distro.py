...
class TestDistribution(TestCase):
    ...
    def test_default_configuration(self):
        ...
        self.assertEqual(
            "process_runtime,os,otel,telemetry_distro,service_instance,containerid,gcp_resource_detector,aws_ec2,aws_ecs,aws_elastic_beanstalk,azure_app_service,azure_vm",
    ...
    def test_configurator_sets_up_opamp_with_http_endpoint(self, client_mock, agent_mock):
        ...
        agent_mock.assert_called_once_with(interval=30, callbacks=mock.ANY, client=client_mock)
...
class TestEDOTOpAMPCallbacks(TestCase):
    def tearDown(self):
        for _logger in Config._get_loggers():
            for handler in _logger.handlers:
                _logger.removeHandler(handler)
    ...
    def test_does_nothing_without_remote_config(self, get_logger_mock):
        ...
        callbacks.on_message(agent, client, message)
    ...
    def test_ignores_non_elastic_filename(self, get_config_mock):
        ...
        client.update_remote_config_status.assert_called_once_with(
            remote_config_hash=b"1234", status=opamp_pb2.RemoteConfigStatuses_APPLIED, error_message=""
        )
        client.update_effective_config.assert_called_once_with(
            {"elastic": {"logging_level": "warn", "sampling_rate": "1.0", "deactivate_instrumentations": ""}},
            content_type="application/json",
        )
        client.build_remote_config_status_response_message.assert_called_once_with(client.update_remote_config_status())
    ...
    def test_sets_deactivate_instrumentations_with_same_rules(
        ...
        config = Config()
        config.deactivate_instrumentations.update(value="*")
        get_config_mock.return_value = config
        tracer_off_config = _TracerConfig(is_enabled=False)
        rule_based_tracer_configurator = _UpdatableRuleBasedTracerConfigurator(
            rules=[(_scope_name_matches_glob("*"), tracer_off_config)],
