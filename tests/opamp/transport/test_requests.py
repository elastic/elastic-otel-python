...
def test_can_send():
    ...
    with mock.patch.object(transport, "session") as session_mock:
        ...
        response = transport.send("http://127.0.0.1/v1/opamp", headers=headers, data=data, timeout_millis=1_000)

        session_mock.post.assert_called_once_with(
            "http://127.0.0.1/v1/opamp", headers=expected_headers, data=data, timeout=1
        )
...
def test_send_exceptions_raises_opamp_exception():
    ...
    with mock.patch.object(transport, "session") as session_mock:
        ...
        with pytest.raises(OpAMPException):
            transport.send("http://127.0.0.1/v1/opamp", headers=headers, data=data, timeout_millis=1_000)
