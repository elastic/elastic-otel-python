...
def test_can_send():
    ...
    with mock.patch.object(transport, "session") as session_mock:
        ...
        response = transport.send(
            url="http://127.0.0.1/v1/opamp", headers=headers, data=data, timeout_millis=1_000, tls_certificate=True
        )

        session_mock.post.assert_called_once_with(
            "http://127.0.0.1/v1/opamp", headers=expected_headers, data=data, timeout=1, verify=True, cert=None
        )
