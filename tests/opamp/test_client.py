...
def test_client_headers_override_defaults():
    client = OpAMPClient(endpoint="url", agent_identifying_attributes={"foo": "bar"}, headers={"User-Agent": "Custom"})
...
def test_build_connection_message_can_serialize_attributes():
    client = OpAMPClient(
        endpoint="url",
        agent_identifying_attributes={"string": "s", "bytes": b"b", "none": None, "bool": True, "int": 1, "float": 2.0},
    )
    data = client._build_connection_message()
...
def test_decode_remote_config(client):
    ...
    decoded = [(filename, config) for filename, config in client._decode_remote_config(message)]
    assert sorted(decoded) == sorted([("application/json", {"a": "config"}), ("text/json", {"other": "config"})])
